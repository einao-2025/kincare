...
const PATIENT_NAV = [
  ...
  { href: '/dashboard/history', label: 'Medical history' },
  { href: '/dashboard/prescriptions', label: 'Prescriptions' },
  { href: '/dashboard/results', label: 'Test results' },
  { href: '/dashboard/imaging', label: 'Imaging' },
  { href: '/dashboard/family', label: 'Family access' },
...
const STAFF_NAV = [
  ...
  { href: '/staff/results', label: 'Upload results' },
  { href: '/staff/progress', label: 'Progress updates' },
  { href: '/staff/audit', label: 'Audit monitor' },
];
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const nav = useMemo(() => {
    if (!user) return [];
    const isStaff = !['PATIENT', 'FAMILY_DELEGATE'].includes(user.role);
    return isStaff ? [...PATIENT_NAV.slice(0, 1), ...STAFF_NAV] : PATIENT_NAV;
  }, [user]);
      ...
      <aside className="w-64 shrink-0 border-r border-clinical-border bg-white p-4 flex flex-col">
        <div className="mb-8 flex items-center gap-2.5 px-1">
          ...
          <div className="leading-tight">
            ...
            <div className="text-[10px] uppercase tracking-wider text-clinical-muted">Patient portal</div>
          </div>
        </div>
